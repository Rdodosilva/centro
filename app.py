...
st.markdown("""
    <style>
        ...
        .css-14xtw13 { display: block !important; visibility: visible !important; }
        .css-14xtw13 > button {
        ...
        section[data-testid="stSidebar"] > div > div > div > div { color: white !important; }
        ...
        /* Hover (não selecionado) mantém efeito ciano suave */
        ...
        /* Fallbacks para .stRadio renderers */
        .stRadio > div > div > div {
            display: grid !important;
            grid-template-columns: 1fr 1fr !important;
            gap: 6px !important;
        ...
        /* ------------------------------
           OVERRIDE FINAL: Selecionado = VERMELHO translúcido
           (colocado com alta especificidade para garantir que apareça)
           ------------------------------ */
        section[data-testid="stSidebar"] div[role="radiogroup"] > label[aria-checked="true"],
        section[data-testid="stSidebar"] div[role="radiogroup"] > label[aria-checked="true"] * {
            /* background com vermelho translúcido */
        ...
        /* Fallback para alguns browsers/renderers que usam role="option" ou data-selected */
        ...
        /* Métricas/cards e demais estilos (mantive o seu padrão) */
        .stMetric {
        ...
        .stMetric [data-testid="metric-container"] > div:first-child { font-size: 0.7em !important; font-weight: 500 !important; margin-bottom: 2px !important; }
        .stMetric [data-testid="metric-container"] > div:nth-child(2) { font-size: 1.2em !important; font-weight: 700 !important; margin: 2px 0 !important; }
        .stMetric [data-testid="metric-container"] > div:last-child { font-size: 0.65em !important; margin-top: 2px !important; }
        .stMetric::before { content: ''; position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(45deg, #00FFFF, #9b30ff); z-index: -1; margin: -2px; border-radius: inherit; }

        .insight-card { background: linear-gradient(145deg, #1a1a2e, #0f0f23); border: 1px solid #00FFFF; border-radius: 15px; padding: 20px; margin: 10px 0; box-shadow: 0 8px 32px rgba(0,255,255,0.1); }
        .trend-up { color: #00FF88; } .trend-down { color: #FF4444; } .trend-neutral { color: #FFAA00; }

        .stPlotlyChart { animation: fadeIn 0.8s ease-in; }
...
# -------------------------------------
# 📥 Carregar dados (suporte a múltiplas abas/anos)
# -------------------------------------
try:
...
# -------------------------------------
# 🎛️ Sidebar: seletores (ano/aba, mês, visual)
# -------------------------------------
with st.sidebar:
    st.markdown("## 🎛️ Filtros")

    # Seletor de ano/aba (se existir)
    year_selected = None
    if sheet_names:
        st.markdown("### 🗂️ Ano / Aba:")
        year_selected = st.selectbox("", options=sheet_names, index=0)
        # atualiza df com a aba selecionada
        try:
    ...
    with col_btn1:
        # apresentação HTML (pequena demo) - você pode manter ou customizar
        apresentacao_html = f"""<!DOCTYPE html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body>
    <h1>Apresentação - Coleta Centro</h1>
    <p>Ano/Aba: {year_selected if year_selected else ''}</p>
    <p>Mês: {mes_selecionado.title()}</p>
</body>
</html>"""
        st.download_button(
            label="📊 PDF",
...
# -------------------------------------
# 🏷️ Header dinâmico (usa year_selected)
# -------------------------------------
# se não houver year_selected, usar 2025 como fallback
...
<div style='text-align: center; padding: 20px 0;'>
    ...
    <div style='color: #00FFFF; font-size: 1.2em; opacity: 0.9;'>
...
# -------------------------------------
# padroniza colunas se não existirem
if "Mes" not in df.columns and "Mês" in df.columns:
    df["Mes"] = df["Mês"].str.lower().str.strip()
elif "Mes" not in df.columns and df.shape[1] > 0:
    df["Mes"] = df.iloc[:, 0].astype(str).str.lower().str.strip()
...
# Cálculo de variação com mês anterior
...
if df_filtrado is None or df_filtrado.empty:
    ...
    # se df_filtrado tem colunas Coleta AM/PM
    if "Coleta AM" in df_filtrado.columns and "Coleta PM" in df_filtrado.columns:
...
with col_left:
    ...
    fig_main.update_layout(
        ...
        legend=dict(orientation="h", yanchor="bottom", y=0.02, xanchor="center", x=0.5,
                    ...
                    bgcolor="rgba(26,26,46,0.8)", bordercolor="rgba(0,212,255,0.3)", borderwidth=1),
        margin=dict(l=50, r=50, t=80, b=50),
        height=450,
        xaxis=dict(showgrid=False, showline=True, linewidth=2, linecolor="rgba(0,212,255,0.3)",
                   color="white", title_font=dict(color="white", size=14), tickfont=dict(color="white", size=11),
                   categoryorder='array', categoryarray=meses_disponiveis),
        yaxis=dict(showgrid=True, gridwidth=1, gridcolor="rgba(0,212,255,0.1)", showline=False,
                   color="white", title_font=dict(color="white", size=14), tickfont=dict(color="white", size=11),
                   title="Quantidade de Sacos", zeroline=True, zerolinecolor="rgba(0,212,255,0.2)")
    )
    fig_main.update_traces(marker=dict(opacity=0.8, line=dict(color="rgba(255,255,255,0.1)", width=1)),
...
with col_right:
    fig_pie = go.Figure(data=[go.Pie(
        ...
        hole=0.6,
        marker=dict(colors=["rgba(0,212,255,0.8)", "rgba(255,107,53,0.8)"], line=dict(color="rgba(255,255,255,0.1)", width=1)),
        textinfo='percent',
    ...
    fig_pie.update_layout(
        title=dict(text=f"⚡ Distribuição AM vs PM<br>{mes_selecionado.title()}", font=dict(size=13, color="#00D4FF"), x=0.5, y=0.95),
        plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)", font_color="white", font_family="Inter",
        showlegend=True,
        legend=dict(orientation="v", yanchor="bottom", y=0.02, xanchor="center", x=0.5, font=dict(color="white", size=9),
                    bgcolor="rgba(26,26,46,0.6)", bordercolor="rgba(0,212,255,0.3)", borderwidth=1),
        ...
        annotations=[dict(text=f"<b style='color:#00D4FF; font-size:18px'>{total_sacos:,}</b><br><span style='color:white; font-size:12px'>Total</span>",
                          x=0.5, y=0.5, showarrow=False, bgcolor="rgba(26,26,46,0.8)", bordercolor="rgba(0,212,255,0.3)", borderwidth=1, borderpad=8)]
    )
...
if "Total de Sacos" in df_linha.columns:
    fig_evolucao.add_trace(go.Scatter(x=df_linha["Mes"], y=df_linha["Total de Sacos"], mode='lines+markers', name='Total de Sacos',
                                     line=dict(color='#9b30ff', width=4, shape='spline'),
                                     marker=dict(size=10, color='white', line=dict(color='#9b30ff', width=3)),
                                     fill='tonexty', fillcolor='rgba(155,48,255,0.15)',
                                     hovertemplate='<b>%{y}</b> sacos<br>%{x}<br><extra></extra>'), row=1, col=1)
...
if "Coleta PM" in df_linha.columns:
    fig_evolucao.add_trace(go.Bar(x=df_linha["Mes"], y=df_linha["Coleta PM"], name='PM',
                                  marker=dict(color='rgba(255,107,53,0.7)', line=dict(color='rgba(255,107,53,0.9)', width=1)),
                                  hovertemplate='<b>PM:</b> %{y} sacos<br>%{x}<extra></extra>'), row=2, col=1)

fig_evolucao.update_layout(height=650, plot_bgcolor="rgba(0,0,0,0)", paper_bgcolor="rgba(0,0,0,0)",
                           font_color="white", font_family="Inter", title_font=dict(size=16, color="#00D4FF"),
                           legend=dict(orientation="h", yanchor="top", y=-0.05, xanchor="center", x=0.5,
                                       font=dict(color="white", size=10), bgcolor="rgba(26,26,46,0.8)",
                                       bordercolor="rgba(0,212,255,0.3)", borderwidth=1),
                           barmode='stack', margin=dict(l=50, r=50, t=80, b=70))

fig_evolucao.update_xaxes(showgrid=False, showline=True, linewidth=2, linecolor="rgba(0,212,255,0.3)",
                         color="white", title_font=dict(color="white", size=12), tickfont=dict(color="white", size=10),
                         categoryorder='array', categoryarray=meses_disponiveis)
fig_evolucao.update_yaxes(showgrid=True, gridwidth=1, gridcolor="rgba(0,212,255,0.1)", showline=True, linewidth=2,
                         linecolor="rgba(0,212,255,0.3)", color="white", title_font=dict(color="white", size=12),
                         tickfont=dict(color="white", size=10))
fig_evolucao.update_annotations(font=dict(color="#00D4FF", size=16, family="Inter", weight="bold"))
...
with st.expander("📋 Ver Dados Detalhados"):
    ...
    if "Coleta AM" in df_display.columns and "Total de Sacos" in df_display.columns:
        df_display["% AM"] = (df_display["Coleta AM"] / df_display["Total de Sacos"] * 100).round(1)
    if "Coleta PM" in df_display.columns and "Total de Sacos" in df_display.columns:
...
<div style='text-align: center; padding: 20px;'>
    ...
    <div style='color: #00FFFF; font-size: 1.1em;'>
        📊 Monitoramento para Otimização da Frota | {header_year}
    </div>
