...
with st.sidebar:
    ...
    with col_btn1:
        # Botão PDF customizado com HTML
        if st.markdown("""
        ...
        <script>
            function downloadPDF() {
                const content = `""" + apresentacao_html.replace('`', '\\`') + """`; 
                const blob = new Blob([content], { type: 'text/html' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = 'Apresentacao_Coleta_Centro.html';
                a.click();
            }
        </script>
        """, unsafe_allow_html=True):
        ...
        # Fallback com download button normal
        st.download_button(
            label="📊 Baixar PDF",
    ...
    with col_btn2:
        ...
        <script>
            function downloadExcel() {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = 'Dados_Coleta_Centro.csv';
                a.click();
            }
        </script>
        """, unsafe_allow_html=True)
